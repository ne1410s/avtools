...
namespace AV.Core.Common
{
    ...
    public sealed class VideoSeekIndex
    {
        ...
        internal void AddMonotonicEntries(TimeSpan streamDuration)
        {
            ...
            while (true)
            {
                var lastEntry = this.Entries[this.Entries.Count - 1];
                var prevEntry = this.Entries[this.Entries.Count - 2];
        ...
        internal long ComputeMonotonicDistance()
        {
            ...
            {
                currentDistance = this.Entries[i].PresentationTime - this.Entries[i - 1].PresentationTime;
